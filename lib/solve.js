...
api.solve = async ({
  ...
  while(noncesRemaining > 0) {
    //console.log('SOLCHECK', currentNonce, noncesRemaining, maxNonces, {graphSize, edgeCount, nonce, noncesRemaining});
    const result = await _engine.solve({
      engine: _engineOpts, input, graphSize, edgeCount, nonce,
    ...
    */
    ...
    for(const solution of result.solutions) {
      //console.log('SOLDIFF', solution.nonce, checkDifficulty({edges: solution.edges, difficulty}));
