 ...
 * @param [graphSize] the size of the graph.
 * @param [edgeCount] the expected number of edges in the solution.
 * @param [difficulty] a 32 byte Node.js Buffer representing a target
 *          difficulty (0 < T < 2^256) that the blake2b hash of the sorted
 *          edgelist (ascending and serialized little endian) must be less
 *          than.
 * @param [nonces] array of 32 bit unsigned integer nonces to start at for each
 *          segment.
 * @param [maxNonces] maximum number of nonces to check for each chain segment.
 * @param [maxChainNonces] maximum total number of nonces to check for in the
 *          chain.
 * @param [chainLength] length of chain to create.
 *
...
api.solveChain = async ({
  ...
  nonces,
  maxNonces = 0xFFFFFFFF,
  maxChainNonces = Infinity,
  chainLength = 1
}) => {
  if(!(Number.isInteger(chainLength) && chainLength >= 1)) {
    throw new TypeError('"chainLength" must be an integer >= 1.');
  }
  const solution = [];
  let nextInput = input;
  let i = 0;
  ...
  while(i < chainLength) {
    ...
    const s = await api.solve({
      engine, input: nextInput, graphSize, edgeCount, difficulty, nonce,
      maxNonces
    });
    if(s === null) {
    ...
    if(i < chainLength) {
      ...
      // update and check total nonces
      totalNonces += (s.nonce - nonce + 1);
      if(totalNonces >= maxChainNonces) {
        throw new Error('Too many chain nonce checks.');
      }
    }
  }
  return solution;
