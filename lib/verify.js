...
api.verify = async ({
  ...
  // check bcrypt of cyclehash against difficulty
  const edgesBuf = Buffer.allocUnsafe(edgeCount * 4);
  for(let i = 0; i < edgeCount; ++i) {
    edgesBuf.writeUInt32LE(solution.edges[i], i * 4);
  }
  const edgesHash = chloride.crypto_generichash(edgesBuf, constants.HASH_SIZE);
  const isDifficult = Buffer.compare(edgesHash, difficulty) === -1;
  if(!isDifficult) {
