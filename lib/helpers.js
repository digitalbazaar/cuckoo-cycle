...
/**
 * Convert integer or array of integers to a uint32 LE buffer.
 *
 * @param input array of integers.
 *
 * @return buffer of uint32 LE values.
 */
function toUint32Buffer(input) {
  const _input = Array.isArray(input) ? input : [input];
  const b = Buffer.allocUnsafe(_input.length * 4);
  _input.forEach((v, i) => b.writeUInt32LE(v, i * 4));
  return b;
};

/**
 * Hash a solution for chaining.
 *
 * @param solution solution object with nonce and edges array.
 *
 * @return buffer hash of solution.
 */
api.hashSolution = solution => {
  // FIXME: document format
  const hash = crypto.createHash('sha256');
  hash.update(toUint32Buffer(solution.nonce));
  hash.update(toUint32Buffer(solution.edges.length));
  hash.update(toUint32Buffer(solution.edges));
  return hash.digest();
};
